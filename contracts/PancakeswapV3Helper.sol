...
import "Nodde/Interfaces/IV3SwapRouter.sol";
...
contract PancakeswapV3Helper {
    ...
    function convertAmountToETH(address token, uint256 amountIn) public view returns (uint256){
    ...
    function swapExactInput(address tokenIn, address tokenOut, uint256 amountIn) external returns (uint256 amountOut) {
    ...
    function getAmountInMaximum(address tokenIn, address tokenOut, uint256 amountOut) external view returns (uint256 amountInMaximum) {
        ...
        PriceInfo memory priceInfo = getPrice(tokenIn, tokenOut);
        if (priceInfo.poolFee > 0) {
            amountInMaximum = _getAmountInDerection(amountInMaximum, priceInfo.price, priceInfo.isDirectOrder, priceInfo.decimals);
        } else {
            PriceInfo memory priceInfo2 = getPrice(WETH9, tokenOut);
            amountInMaximum = _getAmountInDerection(amountInMaximum, priceInfo2.price, priceInfo2.isDirectOrder, priceInfo2.decimals);
            PriceInfo memory priceInfo1 = getPrice(tokenIn, WETH9);
            amountInMaximum = _getAmountInDerection(amountInMaximum, priceInfo1.price, priceInfo1.isDirectOrder, priceInfo1.decimals);
        }
    }
        
    function swapExactOutput(address refundRecipient, address tokenIn, address tokenOut, uint256 amountOut) external returns (uint256 amountIn) {
        ...
        if (amountIn < amountInMaximum) {
            ...
            TransferHelper.safeTransferFrom(tokenIn, address(this), refundRecipient, amountInMaximum - amountIn);
        }
    }

    function swapExactOutputAllIn(address refundRecipient, address tokenIn, address tokenOut, uint256 amountOut) external returns (uint256 amountIn) {
        uint256 amountInMaximum = IERC20Metadata(tokenIn).balanceOf(msg.sender);
        PriceInfo memory priceInfo = getPrice(tokenIn, tokenOut);
        if (priceInfo.poolFee > 0) {
            TransferHelper.safeTransferFrom(tokenIn, msg.sender, address(this), amountInMaximum);
            TransferHelper.safeApprove(tokenIn, address(swapRouter), amountInMaximum);
